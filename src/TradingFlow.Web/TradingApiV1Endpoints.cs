...
public static class TradingApiV1Endpoints
{
    public static IEndpointRouteBuilder MapTradingFlowApiV1(this IEndpointRouteBuilder endpoints)
    {
        ...
        {
            if (!TryParseStrategySelectionMode(mode, out var selectionMode))
            {
                return Results.BadRequest(new ApiErrorResponse(
                    ContractVersions.VersionOne,
                    "strategy_mode_invalid",
                    "Mode must be backtest, paper_experiment, paper_shadow, live, or diagnostic_replay."));
            }

            var strategies = await catalog.GetStrategiesAsync(selectionMode, cancellationToken);
            var response = new StrategyCatalogResponse(
                ContractVersions.VersionOne,
                NormalizeStrategySelectionMode(selectionMode),
                DateTimeOffset.UtcNow,
                strategies.Select(option => new StrategyCatalogItemResponse(
                    new StrategyReference(
                        option.Identity.StrategyId,
                        option.Identity.SemanticVersion,
                        option.Identity.ContentSha256),
                    option.Definition.StrategyName,
                    "swing",
                    option.Definition.Direction,
                    option.Lifecycle.ToString().ToLowerInvariant(),
                    option.Definition.EntryRules.RequirePositiveNews,
                    new[]
                    {
                        option.Definition.Timeframe,
                        option.Definition.Execution.Timeframe,
                        option.Definition.Confluence.Enabled
                            ? option.Definition.Confluence.Timeframe
                            : null
                    }
        ...
        group.MapPost("/universes/preview", async (
            UniversePreviewRequest request,
            IUniverseDiscoveryService workflow,
            CancellationToken cancellationToken) =>
        {
            try
            {
                return Results.Ok(await workflow.PreviewAsync(request, cancellationToken));
            }
            catch (InvalidOperationException exception)
            {
                return Results.BadRequest(new ApiErrorResponse(
                    ContractVersions.VersionOne,
                    "universe_preview_rejected",
                    exception.Message));
            }
        });

        group.MapPost("/candidate-runs", async (
            CreateCandidateRunRequest request,
            ICandidateWorkflow workflow,
            CancellationToken cancellationToken) =>
        {
            try
            {
                return Results.Ok(await workflow.StartAsync(request, cancellationToken));
            }
            catch (InvalidOperationException exception)
            {
                return Results.BadRequest(new ApiErrorResponse(
                    ContractVersions.VersionOne,
                    "candidate_run_rejected",
                    exception.Message));
            }
        });

        group.MapGet("/candidate-runs/{runId:guid}", async (
            Guid runId,
            ICandidateQueryService queries,
            CancellationToken cancellationToken) =>
        {
            var run = await queries.GetRunAsync(runId, cancellationToken);
            return run is null ? Results.NotFound() : Results.Ok(run);
        });

        group.MapGet("/candidate-runs/{runId:guid}/events", async (
            Guid runId,
            long? after,
            ICandidateQueryService queries,
            ICandidateEventStreamService stream,
            HttpContext httpContext,
            CancellationToken cancellationToken) =>
        {
            if (await queries.GetRunAsync(runId, cancellationToken) is null)
            {
            ...
            if (after is null &&
                ...
                !Int64.TryParse(lastEventId.ToString(), out cursor))
            {
                httpContext.Response.StatusCode = StatusCodes.Status400BadRequest;
                await httpContext.Response.WriteAsJsonAsync(new ApiErrorResponse(
                    ContractVersions.VersionOne,
                    "event_cursor_invalid",
                    "Last-Event-ID must be a non-negative integer."), cancellationToken);
                return;
            }

            var pending = await stream.ReadAsync(runId, cursor, 100, cancellationToken);
            if (pending.RequiresResync)
            {
                httpContext.Response.StatusCode = StatusCodes.Status409Conflict;
                await httpContext.Response.WriteAsJsonAsync(new ApiErrorResponse(
                    ContractVersions.VersionOne,
                    "event_cursor_expired",
                    $"Cursor {cursor} is older than retained event {pending.FloorSequence}; reload the candidate run snapshot."), cancellationToken);
                return;
            }
            ...
            try
            {
                ...
                {
                    foreach (var item in pending.Events)
                    {
                        await httpContext.Response.WriteAsync($"id: {item.EventSequence}\n", cancellationToken);
                        await httpContext.Response.WriteAsync("event: candidate.changed\n", cancellationToken);
                        await httpContext.Response.WriteAsync($"data: {JsonSerializer.Serialize(item)}\n\n", cancellationToken);
                        cursor = item.EventSequence;
                    ...
                    else if (DateTimeOffset.UtcNow - lastWriteAt >= TimeSpan.FromSeconds(15))
                    {
                        var heartbeat = new StreamHeartbeatResponse(
                            ContractVersions.VersionOne,
                            $"candidate-runs/{runId:N}",
                            pending.LatestSequence,
                            DateTimeOffset.UtcNow);
                        await httpContext.Response.WriteAsync("event: heartbeat\n", cancellationToken);
                        await httpContext.Response.WriteAsync($"data: {JsonSerializer.Serialize(heartbeat)}\n\n", cancellationToken);
                        await httpContext.Response.Body.FlushAsync(cancellationToken);
                        lastWriteAt = DateTimeOffset.UtcNow;
                    }

                    await Task.Delay(TimeSpan.FromSeconds(1), cancellationToken);
                    pending = await stream.ReadAsync(runId, cursor, 100, cancellationToken);
                }
            }
            catch (OperationCanceledException) when (cancellationToken.IsCancellationRequested)
            {
                // Client disconnect and host shutdown are normal stream termination paths.
            }
        });

        group.MapGet("/candidates/{candidateId:guid}/audit", async (
            Guid candidateId,
            IDecisionAuditQuery queries,
            CancellationToken cancellationToken) =>
        {
            var audit = await queries.GetAsync(candidateId, cancellationToken);
            ...
            CancellationToken cancellationToken) =>
        {
            try
            {
                if (request.ExecutionPolicy.Equals("operator_override", StringComparison.OrdinalIgnoreCase) &&
                    !httpContext.User.IsInRole(TradingFlowRoles.Administrator))
                {
                    return Results.Forbid();
                }
                return Results.Ok(await orders.PreviewAsync(request, cancellationToken));
            }
            catch (InvalidOperationException exception)
            {
                return Results.BadRequest(new ApiErrorResponse(
            ...
            CancellationToken cancellationToken) =>
        {
            try
            {
                return Results.Ok(await orders.ConfirmAsync(request, cancellationToken));
            }
            catch (InvalidOperationException exception)
            {
                return Results.BadRequest(new ApiErrorResponse(
        ...
        group.MapPost("/orders/cancel", async (
            ...
            CancellationToken cancellationToken) =>
        {
            try
            {
                return Results.Ok(await orders.CancelAsync(request, cancellationToken));
            }
            catch (InvalidOperationException exception)
            {
                return Results.BadRequest(new ApiErrorResponse(
        ...
        group.MapPost("/positions/close", async (
            ...
            CancellationToken cancellationToken) =>
        {
            try
            {
                return Results.Ok(await orders.CloseAsync(request, cancellationToken));
            }
            catch (InvalidOperationException exception)
            {
                return Results.BadRequest(new ApiErrorResponse(
            ...
            CancellationToken cancellationToken) =>
        {
            ...
            return Results.Ok(await paperJobs.StartAsync(runName, configPath, cancellationToken));
        });

        group.MapPost("/paper/jobs/{jobId:guid}/cancel", async (
            Guid jobId,
            PaperJobService paperJobs,
            CancellationToken cancellationToken) =>
        {
            var cancelled = await paperJobs.CancelJobAsync(jobId, cancellationToken);
            return cancelled
                ? Results.Accepted($"/api/v1/paper/jobs/{jobId}")
                : Results.NotFound();
        });
        ...
        group.MapPost("/automation/sessions/{sessionId:guid}/cancel", (Guid sessionId, MobileAutomationService automation) =>
        {
            ...
            return Results.Accepted($"/api/v1/automation/sessions/{sessionId}");
        });
            ...
            CancellationToken cancellationToken) =>
        {
            ...
            try
            {
                configPath = configWriter.WriteBacktestConfig(new BacktestRunRequest(
            ...
            return Results.Ok(await backtestJobs.StartAsync(runName, configPath, cancellationToken));
        });

        group.MapPost("/backtests/jobs/{jobId:guid}/cancel", async (
            Guid jobId,
            BacktestJobService backtestJobs,
            CancellationToken cancellationToken) =>
        {
            return await backtestJobs.CancelJobAsync(jobId, cancellationToken) switch
            {
                BacktestCancellationOutcome.Accepted => Results.Accepted($"/api/v1/backtests/jobs/{jobId}"),
            ...
            CancellationToken cancellationToken) =>
        {
            ...
            return Results.Accepted("/api/v1/news/feed");
        });
    ...
    private static bool TryParseStrategySelectionMode(string? mode, out StrategySelectionMode selectionMode)
    {
        switch (mode?.Trim().ToLowerInvariant())
        {
            case null or "" or "backtest":
                selectionMode = StrategySelectionMode.Backtest;
                return true;
            case "paper_experiment":
                selectionMode = StrategySelectionMode.RunPaperExperiment;
                return true;
            case "paper_shadow":
                selectionMode = StrategySelectionMode.RunPaperShadow;
                return true;
            case "live":
                selectionMode = StrategySelectionMode.RunLive;
                return true;
            case "diagnostic_replay":
                selectionMode = StrategySelectionMode.DiagnosticReplay;
                return true;
            default:
    ...
    private static string NormalizeStrategySelectionMode(StrategySelectionMode mode) => mode switch
    {
        StrategySelectionMode.Backtest => "backtest",
        StrategySelectionMode.RunPaperExperiment => "paper_experiment",
        StrategySelectionMode.RunPaperShadow => "paper_shadow",
        StrategySelectionMode.RunLive => "live",
        StrategySelectionMode.DiagnosticReplay => "diagnostic_replay",
    ...
    private static WishlistMarketSnapshot ToWishlistMarketSnapshot(MobileWishlistMonitorSnapshotRequest request)
    {
        ...
        var current = new IndicatorSnapshot(
            ticker,
            timestamp,
            String.IsNullOrWhiteSpace(request.Timeframe) ? "1d" : request.Timeframe.Trim(),
            ...
            BollingerUpper: null,
            BollingerLower: null,
            RelativeVolume: request.DailyRelativeVolume,
        ...
        return new WishlistMarketSnapshot(ticker, current, previous, request.RecentHigh);
    }

    private static MobileWishlistMonitorEvaluationResponse ToMobileWishlistEvaluation(WishlistSwingWatchEvaluation evaluation)
    {
        return new MobileWishlistMonitorEvaluationResponse(
            ...
            evaluation.Reason,
            evaluation.Score,
            evaluation.DailyRelativeVolume,
            evaluation.DistanceFromRecentHighPct,
            evaluation.Ema20ExtensionAtr,
